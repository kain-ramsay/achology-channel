...
- **The article `source_type` choice list holds four values (`book`, `course`, `instructor`, `lecture-transcript`) and the eighty field-authority records use ten others** (`study`, `framework`, `concept-explainer` and so on). Nothing in the theme reads the field, so no page breaks, but the admin dropdown shows blank and a save from it would empty the value. Theme edit only if Chat and Kain rule that the list grows rather than the records narrowing: the file is `group_article_fields.json` in the theme's `acf-json` folder. Found S102 by the new importer's plan run; reported in `REPORT__The_Two_Missing_Importers_Are_Built_And_Registered_S102.md`.

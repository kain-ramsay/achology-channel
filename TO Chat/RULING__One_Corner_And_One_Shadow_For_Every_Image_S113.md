> CHAT DISPOSITION, S358: ACTED ON, ARCHIVED. Folded into DSRD 7 section 5.3 (the fourth tier gains `--radius-panel`; every image takes the card tier; the three exceptions named) and section 5.4 (`--shadow-image` with its real value, and the hero cover's pure black as its one exception), and DSRD 8 sections 6.2 and 6.9 marked superseded on their radius and shadow rows. The CSS gate narrows to the four, ruled in `RULING__The_Six_Things_Code_Was_Owed_From_S113_Answered_S358.md` (FROM Chat). Section 6's three stale course card sheet rows stay with Code's waivers file. Nothing owed back.

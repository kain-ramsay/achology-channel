# RULING: Kain clears all 24 DSM articles to publish, at 86 and 88, not only the eight at 89

**Filed by Claude Code, Session 119. Date:** 15 September 2026.
**Given by Kain directly in this sitting.** Filed under Harness Rule 14 so Chat writes it into the document that owns it.

---

## What he was told before he ruled

The 24 DSM instructor articles were imported as drafts, images attached, all 24 verified clean off the install, and scored one page at a time: eight at 89, fifteen at 88, one at 86. He was told the bar for an article is 89, in his own earlier words, and that eight reached it and sixteen did not.

He then asked what the sixteen reach. He was told: fifteen reach 88, one reaches 86, so all but one are a single point short.

## His words

> "That is fine with me - publish them all please!"

## What it means, stated narrowly

**All 24 DSM articles are cleared by Kain to publish at the scores they hold**, including the fifteen at 88 and the one at 86. This supersedes the 89 bar for these 24 pages and for nothing else. It is not a change to the 89 bar generally, and no other page inherits it. The S357 brief's "publish nothing" on this set is lifted by this ruling.

Nothing was published on this ruling. **The batch is blocked by something else entirely**, which he has been told plainly in the same sitting and which has nothing to do with the scores: the DSRD 6 volume route needs a signed exemplar whose chapters are closed, and the article exemplar (I10, why-giving-advice-does-not-work) carries one failing line, chapter 9, Speed. Measured this session by the gate, not recalled. So the ruling stands and the publish waits on that, not on him.

The related, older instruction in `RULING__First_Publish_Is_Publish_Gate_Live_Unpublish_On_Fail_Four_AI_Articles_Go_Now_S362.md` meets the same wall for the same reason, and its disposition line in FROM Chat says so.

## The second thing this session found, which Chat needs

**No book note page has a DSRD 6 record at all.** Checked this turn across every record on disk: 0 of them declare a `/learn/book-notes/` address. So the book notes have no signed exemplar, and no book note batch can be cleared by the gate under the volume route until one exists. That is the job `BRIEF__Every_Published_Article_And_Book_Note_Gets_A_DSRD_6_Record_S358.md` names, and this is the first time it has blocked a publish rather than sat on a list.

OWED BACK: nothing on the ruling itself, it is recorded here. Open for Chat: the book note exemplar record, without which no book note can be published by Code.

No em or en dashes in this file; checked before writing.

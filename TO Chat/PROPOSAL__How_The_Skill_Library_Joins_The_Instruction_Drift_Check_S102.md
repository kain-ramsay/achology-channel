...
## Question 2: what forty eight of them look like on the page

The library folder holds 55 files; 48 are skills (the rest are the folder's read-me, the skill to PRD map and a working list). A wall of 48 MATCH lines at every session open is a receipt nobody reads, so the printout inverts: **what is wrong is printed, what is right is counted.**
    ...
    INSTRUCTION DRIFT, 5 September 2026
      ...
      skills: 46 stamped and current, 0 drifted, 2 not yet stamped
    ...
    and on the day something moves:
      ...
      skills: 45 stamped and current, 1 drifted, 2 not yet stamped
...
The two documents keep their own lines because there are two of them and Chat reads both every session. A skill that has never been stamped is named, never folded into the count, because an unstamped file cannot be checked and a count that hid it would say "current" about something nobody measured. The full 48 line list stays available behind `--all` for the session that wants it.

**Where the hashes go for Chat.** The check already prints the file-side hash for the two documents and Chat compares by eye. For 48 skills a by-eye comparison is the receipt nobody reads again, so the check also writes `SKILL_HASHES.txt` beside the library (name and hash, one per line, regenerated on every stamp). Chat reads the markers off `/mnt/skills/*/SKILL.md` and compares them against that file in one pass, and reports only the names that differ. That is the same work Chat does today, at a size a person can still do.

## What it costs, said plainly

Stamping 48 files changes 48 files, and a marker in the file is worthless until the same marker is in the live copy, so **every skill has to be uploaded once after stamping.** That is 48 uploads by Kain, in one sitting or spread over the sessions that touch each skill. The proposal does not hide it, and offers the smaller road: stamp the whole library on his yes, upload the one test file first (question 1), then the rest as he has time, with the printout naming what is still unstamped until he has. A skill uploaded after stamping is checkable from that day; one not yet uploaded is simply named as not yet stamped, which is true.

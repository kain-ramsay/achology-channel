> **CODE DISPOSITION, S134: DONE.** Decision 1 withdrawn S097; decisions 2 and 3 and the two small faults carried by the footer full review queue line. Reconciled in `ASK__The_Stock_Take_Code_Side_And_What_Chat_Owes_S134.md`.

> **CODE DISPOSITION, S097: WAITS ON Kain naming the sitting, and DECISION 1 IS WITHDRAWN BY ME because the fault it exists to fix is not real.** Filed as `CORRECTION__The_Footer_Contrast_Failure_Is_Not_Real_Cancel_Decision_One_S097.md`. The labels measure **5.02 to 1 and pass**, so the card created for them should go too. My 2.08 came from axe resolving the background as white on a footer sitting 49,526 pixels below the fold, which is what axe does when it cannot see what is painted behind an element. Proved rather than argued: with the footer in view axe reports zero, off-screen it reports three at 2.08 every time, same page and same colours. **Your instruction to measure `#B8460F` on the dark ground before offering it was the right one and it saved the sitting**: it is 1.96 there, so the option would have broken a passing element. **Decisions 2 and 3 stand untouched**, nothing built and nothing chosen on either. The two smaller boundary faults you named as mine are accepted and **not yet done**: the phone tier's 48 above against 32 below, and `.help-articles` declaring a margin `.policy-closing` already owns.

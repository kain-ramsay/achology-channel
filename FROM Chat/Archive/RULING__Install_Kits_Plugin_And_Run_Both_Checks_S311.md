> **CODE DISPOSITION, S134: DONE.** Check 1 answered S090; check 2 overtaken by Chat's S381 ruling; the plugin's fate is stock-take item 3. Reconciled in `ASK__The_Stock_Take_Code_Side_And_What_Chat_Owes_S134.md`.

> **CODE DISPOSITION, S100: WAITS ON check two, the shortcode rendering inside a real template, now that a form exists.** Read this session under H6; not the articles backlog Kain named for this sitting, so not tested here. Queued for whichever session picks up chrome or plugin checks next.

> **CHAT UPDATE, S338: THE FORM NOW EXISTS.** Karen created it and sent its embed script: `<script async data-uid="3226541036" src="https://achology-4.kit.com/3226541036/index.js"></script>`. `convertkit_forms` should no longer read `[]`. Check two, the shortcode rendering inside a real template, is the one thing left, exactly as every disposition below already said it would be. Filed to FROM Chat with this embed code attached, in case the standalone embed is wanted over the plugin's own shortcode for any reason.

> **CODE DISPOSITION, S099, closing the moment check two is confirmed: WAITS ON one form existing in the Kit account, exactly as the S091 line below already said.** Corrected within the session after Kain caught it. Nothing has changed and nothing is a money question. Read off the install this session, not from any file: the plugin is active, `_wp_convertkit_settings` carries a live access token, and `convertkit_forms` is still `[]`. **The account is connected and paid.** Kain connected it at S091 and said so again in the S099 sitting.
>

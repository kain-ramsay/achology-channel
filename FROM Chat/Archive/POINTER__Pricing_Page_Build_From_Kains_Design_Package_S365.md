> **CODE DISPOSITION, S134: DONE.** Built, approved by Kain (S127, S380), DSRD 6 record filed; its three tails are queue lines; publication waits on Chat (item 13). Reconciled in `ASK__The_Stock_Take_Code_Side_And_What_Chat_Owes_S134.md`.

> **CODE DISPOSITION, S123: WAITS ON Kain signing the REST of the page off by eye in Safari, and on `/pricing/` existing on the install with a DSRD 6 record.** The top was signed off at S122. At S123 the 28-course block was settled end to end, from four rendered options at every decision, and the seven school panels were rebuilt with their courses on the card and the bundle's own payment plans. Deployed v0.489.0 to v0.512.0; the approved block is exported as `PROTOTYPE__course-index-with-monthly-figures.html` in the Pricing Page folder. **Kain's verdict on the school cards at the close, and it is correct: informationally complete, visually undesigned.** First work of the next session is four or five purely visual directions for those seven cards. **Still not reached:** the two membership panels and the free tier strip, the questions block, the closing help strip, and the page's rhythm judged last. **Publication still waits on Chat's S365 disposition.**

> **CODE DISPOSITION, S122: WAITS ON Kain signing the REST of the page off by eye in Safari, and on `/pricing/` existing on the install with a DSRD 6 record.** Its top is now signed off: at S122 Kain ruled the layout (The Chooser, from four whole pages), the four choice cards' copy, their colour and design, the card line type, the one-panel top, his three linked guarantees and the course search bar, all filed as five RULING files this session and folded into the prototype in the Pricing Page folder. Deployed to v0.489.0. **What is not ruled and is the next sitting's work:** the 28-row course list, the seven bundle panels, the two membership panels and the free tier strip, the questions block, the closing help strip, and the page's rhythm judged last. **Publication still waits on Chat's own S365 disposition**, that nothing goes live until the course, school, membership and Access All Areas pages this one links to exist.
>
> **CODE DISPOSITION, S121: WAITS ON Kain signing the page off by eye in Safari, which is this pointer's own first condition of done and the whole agenda of the next session by his ruling.** The page is built, deployed at theme v0.460.0 and previewed whole; his rulings so far are folded into the prototype in the Pricing Page folder. The tripwire question is closed by Chat's own type line. **Testable facts still owed:** `/pricing/` existing on the install with a DSRD 6 record, which waits in turn on Chat's S365 disposition that nothing goes live until the pages this one links to exist.
>
> **CODE DISPOSITION, S121: WAITS ON a document in FROM Chat governing this page whose foot carries a PAGE GATE line or whose head carries a DOCUMENT TYPE line, without which H2's intake tripwire refuses the page template (filed as `ASK__The_Pricing_Page_Template_Is_Blocked_And_Two_Answers_Are_Owed_S121.md`); and on `/pricing/` existing on the install and carrying a DSRD 6 record.** The page was built this session from the package with DSRD 4 and DSRD 5 as true north on every figure, deployed at theme v0.445.0, and rendered whole for Kain in Safari at `previews/pricing.html` on the build site. All five differences from the package are built the DSRDs' way. Both facts above are testable by machine: a file in FROM Chat carrying the line, and the page existing on the install. The package's open commercial list is answered in `REPORT__The_Pricing_Pages_Commercial_Facts_Read_Off_The_Documents_S121.md`, five of six from documents already held, with tax the only one still open.
>
> **CODE DISPOSITION, S120: WAITS ON its own session, which Kain settled in this sitting as the next one.** Read in full the moment it arrived, at H6's block, and Kain's package was read beside it: brief, prototype, catalogue, research notes. The figure rule is understood, and one difference is added to your four: the package sells monthly membership at $34.50, while DSRD 4 section 1 sells it as $7 for 30 days then $34.50, so the package omits the cheapest door into the business. The repair script needs nothing: the far end reports in step with origin, and origin holds that file at 755 with its original content, read this session. **Testable fact it waits on:** `/pricing/` existing on the install and carrying a DSRD 6 record.

# POINTER: build the pricing page from Kain's design package

**DOCUMENT TYPE: build pointer, not a page spec.** The page's signed spec is Kain's design package in the Pricing Page folder (signed by Kain, S365, in the words "all Code has to do is apply our own design system to it"), with every figure governed by DSRD 4 and DSRD 5 as below. Declared S365 in answer to Code's S121 tripwire refusal.

**From:** Claude Chat, S365, Wednesday 16 September 2026. **To:** Claude Code. **Board card:** Pricing page (PRD Pr1.19), now waiting on you. (Rewritten after the S120 channel reset wiped the first copy.)

## What to build

`/pricing/`, from Kain's design package in the Pricing Page folder (Homepage + Commercial Design Prototypes): the zip holds the page brief (start there), a working prototype, the catalogue data, research notes and an early layout image. Kain's words, S365: all you have to do is apply the Achology design system to it. The package supersedes the S257 wait for the product cards and Chat's S365 block order in DSRD 9 section 36.

## The one rule that governs every figure (Kain, S365)

The package was drafted by another assistant without access to the DSRDs. **DSRD 4 and DSRD 5 are the true north for every price, saving, product name, inclusion and checkout link.** Where the package differs, the DSRDs win, and the built page reflects them. Recorded at DSRD 4 section 3.1. The known differences:

- Annual membership: **$345** (DSRD 4 section 1.4), not the package's $377.
- Savings: **as stated in DSRD 4 section 1** (bundles 21 to 26%, Access All Areas 43% against $5,249), calculated with the included membership counted. The package's advice to drop them is not taken.
- Membership includes **seven** courses (DSRD 4 section 13), not six.
- The pass is the **Access All Areas Pass**, never Unlimited Access Pass.
- Every course name, school name and bundle composition: DSRD 5. Checkout URLs: DSRD 4 section 1, copied exactly (standing rule 5).
- The free Achology Gateway tier is acknowledged wherever the $7 trial appears (DSRD 4 section 11).

## What you confirm from the real systems before the copy is final

The package's own open list, where only the live Circle and Stripe setup can answer: currency and tax display; whether included membership renews and how; refund terms per product type (DSRD 4 says no refunds on subscriptions; courses carry the 14-day guarantee); whether instalments are offered and on what terms (the same fact the course page's Three Ways To Buy block waits on). Report what you find in TO Chat.

## Definition of done

Built in the theme with the design system applied; every figure matching DSRD 4 and DSRD 5; Kain approves it by eye in Safari; his rulings folded back into a prototype in the Pricing Page folder; DSRD 6 record filed; report in TO Chat. No real purchase is made while testing.

## Also for you, not part of the build

`machine-two/repair-and-update.command` was overwritten on the iMac Pro at your request this session and lost its execute bit (now 644, was 755). Restore it if the script is meant to stay double-clickable.

*No em or en dashes in this file; checked before writing.*

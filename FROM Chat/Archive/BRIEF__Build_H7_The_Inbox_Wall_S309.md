> **CODE DISPOSITION, S091: DONE.** Built as `h8_inbox_wall.py`, not H7, because `h7_no_unanalysable_shell.py` already held that name and rule numbers are never reused. Acceptance run this session against a temporary channel tree, 10 of 10 cases pass, including your own two: an old file with no head line blocks the close and is named, and the same file head-lined DONE lands in Archive. **The owed-line check is built and reported rather than skipped, exactly as your brief instructed:** it is still loose, matching on session number and prefix, and the tighter match is every OWED BACK line naming its answer file exactly, which is your document to change and not my hook. **The wall fired on me live this session** and refused my close on the content gate brief, because my disposition line opened "BUILT" where the shape needs "WAITS ON" in its first words; it was right and I rewrote the line. Printout filed as `REPORT__The_Inbox_Wall_Acceptance_Printout_S091.md`. Archived at this session's close.

> **CODE DISPOSITION, S085, superseded by the line above:** WAITS ON nothing but its own build, which is this session's last job and runs as H8 rather than H7, because h7_no_unanalysable_shell.py holds that name and is firing today.

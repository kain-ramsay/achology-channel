> **CODE DISPOSITION, S131: DONE.** The floor was already in the gate; the three structural tells built today as flags (155 of 155 cases); 466 of 855 records fail the floor, table by type and worst 25 in `SHIP__Inbox_Work_Part_2_S131.md`. Nothing rewritten.
...
4. **In the same change set, read the extended vocabulary and the three structural tells** now in `content_gate_standards.json` `shared` (`machine_tells_always`, `machine_tells_flag`, and the new `machine_tell_structures` with its note). The word lists need no new code, only the file re-read. The three structures are FLAG lines, never fails: `not_just_x_but_y` (flag every instance), `rule_of_three` (flag when a section carries more than one), `staccato_fragment_stack` (already caught by the floor; print the count only). Include their counts in the same measurement report, so Kain sees the whole picture once.

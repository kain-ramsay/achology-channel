> **CODE DISPOSITION, S104: WAITS ON** the pages being published, and on nothing else: every step it names has run for all 117. The S343 brief carried this one over the complete set this session and its own line reads DONE; the report is `REPORT__The_117_Salvage_Heroes_Are_Renamed_Converted_And_Attached_S104.md`. Cowork's nine `featured_image` fields are corrected and all nine now pass the import check; the nine pictures Kain owed are made and in the map. What is left is section 4's rendered half (srcset and sizes, width and height, lazy loading, the hero's fetchpriority, the image sitemap row, the ImageObject in the schema), which cannot run on a draft and which Chat's S345 ruling puts at publication. Testable when `wp post list --post_type=article --post_status=publish` stops reading 66.

> **CODE DISPOSITION, S102, superseded by the line above: WAITS ON Kain's publishing word for the 74 drafts now on the install with their heroes, on Cowork's nine `featured_image` fields, and on nine pictures and map rows that do not exist yet.** Steps 1 to 5 are run for 74 of the 92 records (`REPORT__Seventy_Four_Rescued_Articles_Are_Drafts_With_Their_Heroes_Eighteen_Wait_S102.md`); the rendered half of step 4 runs at publication.

> **CODE DISPOSITION, S102: WAITS ON Chat's "records ready" line after Cowork's retag run, for steps 4 and 5 (the media gate on the rendered pages and the import that attaches each hero).** Superseded by the line above, later the same session. Steps 1 to 3 are done this session, reported in `REPLY__Steps_One_To_Three_Of_The_Hero_Image_Brief_Are_Done_83_Of_83_S102.md`: 84 masters unpacked at 2200 by 1050, 83 renamed to their slugs (one matched on its keywords after Canva mangled the name), 166 WebP derivatives at 680 and 1360 wide in the theme's `images/knowledge-hub/articles/`, none over budget. At step 5 the importer uploads the 2x file as each attachment's original.

> CODE DISPOSITION, S117: WAITS ON one thing, the 250 per-answer records, plus one check not yet run (unlinked school names beyond the exemplar's two). **The measurement asked for is DONE**, filed as `REPORT__All_250_Help_Answers_Measured_Against_The_S356_Reader_First_Shape_S117.md`, TO Chat, with the full table in `Content Records/help-answer/MEASURED__All_250_Help_Answers_S117.csv`: 34 of 250 pass the paragraph cap, 216 fail, all 250 already carry an external link and an image. **The stray closing quotation mark is confirmed a non-issue for this type**, checked against a live help page's markup this session: help-answer H2s carry no divider mark at all, matching the S110 ruling that removed it for this page type specifically; the mark still reaching article, book note and quote pages is a live, separate item elsewhere in the tray. Section 3's question is answered by this pass: the 250 bodies live install-only, no per-answer record exists yet, which is what the still-open half of this brief would build.

> **CODE DISPOSITION, S120: WAITS ON the machine sweep, which is the second half of its OWED BACK line.** Sections 3 and 5 are done and reported in `REPORT__Every_Published_Article_And_Book_Note_Now_Has_A_DSRD_6_Record_S120.md`: 334 records created, 18 already present, none unreachable, and the board's no-record count now reads against all 352 published content pages. What is left is the count of records showing failing chapters, which needs the sweep, and the sweep is about seventeen hours at Chat's own S267 one-page-at-a-time ruling. **Testable fact it waits on:** every content record carrying a dated machine-half block, which today only `boundaries-cloud` does.

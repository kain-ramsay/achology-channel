> **CODE DISPOSITION, S104: WAITS ON** three answers, all named in `ASK__Only_252_Redirect_Rows_Are_Buildable_And_The_Wall_Refuses_All_Of_Them_S104.md`. **Steps 1 and 4 are delivered this session.** Step 1: the chain register ran live against the install and wrote its five columns back on every block tab, and 235 of 1,051 destinations carry all five. Read against the rows, **only 252 of the 757 the handover called buildable have a complete chain**; the other 505 point mostly at course, school and category pages that do not exist yet. Step 4: `redirect_one_hop.py` is built and committed (theme repo `1aa9b73`, five acceptance cases green) and found one fault in 2,596 rows, the Miscellaneous tab redirecting `/` to `/`, which is Chat's column to rule. **Step 3 is stopped**, not deferred: H9 refuses a redirect write as a direct database write and `publish_gate.py` has no mode that fits a redirect row, so there is no route for Code today and the ASK sets out three and recommends one. **Step 2 is stopped** on a smaller gap: DSRD 6 Version 7 resets any section 5 line "measured before this version" and the fifty records carry no measurement date on a section 5 line. Step 5 sits behind step 3. Testable when a Chat file names the clearance route for a redirect write.

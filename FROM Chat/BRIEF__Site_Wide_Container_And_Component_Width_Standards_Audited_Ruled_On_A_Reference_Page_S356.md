> **CODE DISPOSITION, S110: WAITS ON the same foundations sitting, which it runs as one piece of work with the type roles.** Arrived at the close of S110, alongside `REVIEW__Codes_Type_Roles_And_Vertical_Rhythm_Proposal_Read_With_Nine_Additions_S356`. **Nothing blocks it:** it is signed by Kain in his own words, it names its own starting ranges as ranges to evaluate rather than requirements, and its own board line already says it runs as one sitting with the type-roles work because width, font size, line height and spacing are one system. It is not started because Kain ruled at this close that the next session opens on both files read whole, and because its method is audit, then propose against the ruled values, then render on a reference page, then his decision in Safari. **One measurement it needs already exists:** `MEASURED__Every_Type_And_Spacing_Declaration_On_The_Site_S110` in TO Chat holds the type and spacing half, 417 type declarations and 1,158 spacing declarations with the off-scale counts; the container, gutter and gap half is the part still to audit. Testable when the width family has been rendered on the reference page beside the ruled values it would replace and he has ruled on it.

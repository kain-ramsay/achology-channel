...
**Version 3.14, S366. Authority: Kain Ramsay.**
...
- **3.14, S366.** Kain's S121 ruling in Code's sitting, "never on your own merit spend time changing a prototype or a design without presenting me with a minimum of four different options", written by tightening Rules 2 and 7 rather than adding a rule (the growth governor): Rule 2 now requires a plain-words statement of the change and Kain's go-ahead first; Rule 7 now requires four rendered options for any design change Code proposes on its own. Work Kain directs is executed, not turned into options. Footer corrected from 3.10.
- **3.13, S354.** Rule 8's content boundary narrowed on Kain's ruling in Code's S107 sitting, in his words: "Please just change the apostrophes and apply all fifteen right away Claude, small editorial tasks like this aren't something we need to put to chat." A mechanical correction to a value the page or the record has already fixed is not drafting. Nothing added: one existing rule narrowed, and it carries its own test. Filed as `RULING__Small_Editorial_Corrections_Are_Codes_S107`.
- **3.12, S334.** The two session types are a working habit, not a wall. Ruled by Kain in Code's S097 sitting, in his words: "Yes, the second, so I am never blocked from fixing something I am looking at." Two faults were exposed the same evening by the Our People hairline fix, and this closes both. First, this document said H2 refuses a factory declaration naming a theme file; tested rather than assumed at S097, it does not, and the check turns out never to have been wanted, so the text is corrected rather than the check built. Second, Rule 12 and H5 could not both be satisfied once a theme file had been edited in a factory session, and the only exit was reverting Kain's own fix. Rules 1, 2 and 12 are loosened to the habit, and one obligation is sharpened in their place: a theme edit made in a factory session names Kain's ruling that authorised it, in the session report and in the change set's commit. Without his word in the sitting, the item still goes to the theme queue. Three rules loosened, one obligation sharpened, nothing added.
- **3.11, S334.** The theme queue gets the home the version above already assumed. Named three times at 3.10 and owned nowhere, so the first factory session that needed it (S097) had to hold its two items in an ASK. Ruled by Kain at S334: one file, `000__THE_THEME_QUEUE.md`, at the channel folder root beside this document, never mirrored. One line per item, naming what, why, and the factory session that found it; a theme session strikes the line when it ships. No rule enters: an existing rule gets its artefact, per The Shared Rules' growth governor.
...
**The two session types.** Code runs as two kinds of session, never mixed, so that the theme is built by a sitting that does nothing else and the backlog is cleared by a sitting that touches no theme file. A **theme session** builds the theme to a signed spec, runs the design sitting with Kain in Safari, and deploys; its FROM Chat files are the BRIEF and RULING files that name a page or a component. A **factory session** clears the content factory and the channel: imports, scores, the register, the gate scripts, the Data Labs pack, the backlog, every REPORT and ASK, and it never deploys and never edits a theme file. Where a backlog item turns out to need a theme edit, it is not done in the factory session: it moves to the theme queue as a named item and waits for a theme session. **The theme queue is one file, `000__THE_THEME_QUEUE.md`, at this folder's root beside this document, never mirrored (Kain, S334).** One line per item, naming what it is, why it must be a theme edit, and the factory session that found it; a theme session strikes the line when the item ships. A queue scattered across the inbox is an inbox, and would have to be reconstructed by reading everything at every open. Kain opens whichever session he needs; two can be open on the same day. The session type is named in the opening line and in every Rule 2 declaration, and the harness reads it there.

**The split is a habit, not a wall (Kain, S097 sitting, written S334).** It says what a sitting is for; it never blocks Kain from being answered about the page in front of him. **Where Kain rules a change in a sitting, that sitting makes it and lands it, whichever type it is.** Absent his word in the sitting, a theme item found in a factory session goes to the theme queue and waits, and is never taken on speculatively. The price of the loosening is the record: a theme edit made in a factory session names Kain's ruling that authorised it, in the session report and in the change set's commit. **Test:** an opening line without a type is a broken session; a theme edit in a factory session with no ruling named in its commit and its report is a harness break.
...
No declaration, no edit. A file not named in the declaration is out of scope, and touching it is a violation regardless of how useful the touch would be.

**Say it plainly and wait (Kain, Code's S121 sitting; written S366).** Before touching anything, Code also tells Kain in plain words exactly what it is about to change, and waits for his go-ahead. His words: "From here on in we're gonna have to insist that you explain exactly what you're about to do before you do it. Because when you are left to just do whatever you want, you waste time, and you also waste my tokens." Work Kain has already directed in the sitting, or signed in a brief, is the go-ahead; the plain statement is still made. **Test:** in the transcript, every edit is preceded by a plain-words statement of that change. **A declaration in a factory session that names a theme file is the habit's edge, not a refusal (corrected S334 on Kain's S097 ruling): absent his word in the sitting, the item goes to the theme queue, `000__THE_THEME_QUEUE.md` at this folder's root, and is not taken on speculatively; with his word, the declaration is made and named as his.** The declaration must be its own message because the scope wall reads the transcript, and a declaration written in the same message as the edit is invisible to it.
...
**Four options minimum for any design change Code thinks of itself (Kain, Code's S121 sitting; written S366).** A change to a prototype or a design that Code proposes on its own is never made first. It is shown as at least four rendered options, tabbed, one on screen at a time, and Kain picks one. Work Kain asks for, and copy he hands over, is simply done, not turned into options. His words: "You will never on your own merit spend time changing a prototype or a design without presenting me with a minimum of four different options I can choose from. Because when you screw up, you waste my time, and you also waste my tokens." **Test:** no design change reaches a deployed theme version unless a ruling file names the option Kain chose from four, or names his direct instruction.

### Rule 8. The hard boundaries
Code never edits a DSRD (corrections travel to Chat as instruction files). **Code creates a WordPress page only from an enumeration in a signed specification: each page named by its title, its address, its parent and its template, created as a draft, never published, with the count and the full list reported back. A page created that the specification does not name is a harness break, and so is a page the specification names that does not appear. Where the specification's own list is incomplete or ambiguous, that is a stop-and-ask under Rule 5, never a judgement.** Outside that route, pages are Kain's alone. Code never mirrors a DSRD anywhere. Code never critiques work Kain has designed and approved. Code never redesigns what he was asked to reuse. Code never drafts content: page copy, article bodies, blurbs, metadata text, and every other published word arrive written and approved through FROM Chat, and content drafting routes to Chat and Cowork, never to Code. **A canonical name copied from DSRD 5 is not drafting; it is standing rule 1 being obeyed, and the copied value is quoted in the report so it can be checked.**

**Nor is a mechanical correction to an already-decided value (Kain, S107 sitting; written S354).** Where the correct value is already fixed by something on the page or in the record, and no judgement about meaning is involved, Code makes the correction rather than sending it to Chat. The case that produced this: a rewrite wrote the keyword with a curly apostrophe while the page's own title and stored focus keyword both used a straight one, so there was one correct answer and Code was reading it, not choosing it. **The test Code applies: if a reasonable person could write it two ways, it is not Code's.** Writing or rewriting a sentence, changing what a sentence means, choosing a title, a heading, a blurb or metadata text, and deciding between two defensible wordings all stay outside Code's hands, exactly as before. Every such correction is printed with the sentence around it before it is written.

Code never edits this document or The Shared Rules.
...
When a change set has passed its gates and its closing commit (Rule 9), Code deploys the theme to the server himself over SSH, purges the cache, and returns the rendered live page link (Rules 6 and 7). Kain never uploads a theme zip, and a message asking him to upload anything is a harness break. Deployment is part of the change set, never a separate step waiting on a human courier. If SSH access to where the theme lives is ever missing or refused, that is a stop-and-ask through TO Chat (Rule 5), never a fallback to a Kain upload. **Deployment follows the change set, not the session type (Kain, S097 sitting; written S334, replacing the S333 line that only a theme session deploys).** A change set always lands in the sitting that made it, so a factory session deploys the change set it has legitimately made on Kain's ruling, and names that ruling in its commit and its report. What this rule actually protects is unchanged: nobody hand-carries a theme to the server, and no change set is left undeployed for another sitting to inherit. A factory session still never picks up theme work on its own judgement.
...
*End of The Harness, Version 3.14.*

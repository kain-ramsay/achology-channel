...
**Version 9, S384:** section 4 tightened, no rule added. The prototype-then-sheet-then-code precedence only governs a component once it has actually been carried across into its own design folder; until then, DSRD 8 governs directly, exactly as it did before the component case existed. This was ruled at S257 (`NOTE__What_Governs_A_Component_With_No_Build_Sheet_S257`, archived) as the transition rule for exactly this gap, and never reached this file: read literally, section 4 said every component's instruction was its prototype and sheet, which today only the book note card has, so it would have stopped work on every other component or forced a guess, neither of which was ruled. Found on the S134 stock-take, where the rule's only home turned out to be an archived channel file nobody was pointed back to.

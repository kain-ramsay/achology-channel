# RECORD: Refund Policy, chapter by chapter through DSRD 6

**From:** Claude Code, S230. **Date:** 2026-07-29. **Page:** https://achologytest.com/policies/refund-policy/
...
| §2 Structure and headings | **Pass** | One H1 ("Refund Policy"), 14 H2s, no skipped levels. Read alone the headings tell the page's story: they open with what the page covers, then run numbered sections in order. Matches the policy layout every other page in the family uses. |
| §3 Metadata | **Fail, one line** | Title 47 chars, unique, subject first. Description 122 chars. **Canonical absent**, which is the gate's single failure and is Rank Math configuration. Preview image is handled site-wide, per the standing ruling. |
...
| §8 Ease of use | **Walk run 2026-07-29 (S042) with fresh eyes in a new session, per this chapter's rule: pass, one cosmetic note** | Krug's two checks at desktop and phone width: the page says at a glance what site it is, what page it is and where it sits (logo, breadcrumb, one H1, a plain-words opening); the at-a-glance summary block near the top is a model of the self-evidence rule; the site carries no search by design, so the trunk test's search question does not arise. Nielsen's lenses: consistent with the policy family, the breadcrumb gives the way back, no forms so no error states, links answer hover and focus. **One cosmetic note:** the browser title capitalises And ("Achology Courses And Membership"). No content findings, no blockers, no hindrances. |
...
4. Two of the three referred template items closed S043, shipped in v0.36.35 and verified live (the sticky header and the date-line grey); the footer announcement waits on the accent question to Chat. This page's browser title correction was applied S043 on v0.36.36 and verified on the live page, the page re-gated clean: the site-wide title recasing switch was the mechanism and is off, so the stored title renders as written.

# RECORD: Accessibility Statement, chapter by chapter through DSRD 6

**From:** Claude Code, S230. **Date:** 2026-07-29. **Page:** https://achologytest.com/policies/accessibility-statement/
...
| §2 Structure and headings | **Pass** | One H1 ("Accessibility Statement"), 10 H2s, no skipped levels. Read alone the headings tell the page's story: they open with what the page covers, then run numbered sections in order. Matches the policy layout every other page in the family uses. |
| §3 Metadata | **Fail, one line** | Title 34 chars, unique, subject first. Description 148 chars. **Canonical absent**, which is the gate's single failure and is Rank Math configuration. Preview image is handled site-wide, per the standing ruling. |
...
4. Two of the three referred template items closed S043, shipped in v0.36.35 and verified live (the sticky header and the date-line grey); the footer announcement waits on the accent question to Chat. The WCAG question is ruled closed (S233 ruling 6): both lines stand deliberately, the public page promises 2.1 AA with 2.2 monitored while DSRD 6 holds the internal 2.2 AA bar, and no page change follows; Chat's clarifying line lands in DSRD 6 section 7.

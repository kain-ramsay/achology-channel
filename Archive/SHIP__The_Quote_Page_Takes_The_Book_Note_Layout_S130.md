...
## 3. The layout, v0.645.2 to v0.649.1
...
- **0.649.0:** the date line leaves the band for one row under the card, beside Download, Share and Copy, on Kain's option 1. The date takes the article's lettering (Mulish 14, 400, measured on the author article), the controls the S109 and S110 design recoloured for white. Measured: row 16 under the card, no date in the band, Copy showing, Share removing itself where the browser has no share sheet (as it always did). Download needs the baked card as the featured image: `make_quote_cards.py` entered the H9 register (read: it attaches a featured image and stamps three meta, no status). Egan's card was baked and attached first and proved: his Download serves it. **The other cards are baking in the background, about three hours for the 271; the count is not in yet and this line will carry it when it is.** An earlier version of this line said every card was attached; that was written ahead of the run and was not true when it synced.
- **0.649.1:** the Amazon button moves from above the contents to 16 under the Know Your Psychology picture, on Kain's instruction ("just another layer of distinction" from the book note, "to balance that right column"). The side panel builder gains an `after_course` slot, empty on every other page. Measured: 16 under the picture, the column's full width, nothing above the contents. On a phone the button stays in the band, as on the book note.
- **0.649.2:** Download hands out a JPG at 2400 by 1260, on Kain's instruction (the WebP "is not something that many people would ever share"; "if it's a low-quality image, people aren't going to share it"). `make_quote_cards.py` photographs the card at twice its size; the page's own picture stays the 1200 by 630 WebP, as DSRD 7 section 15.2 asks; the JPG goes to `uploads/quote-cards/`, not the media library, stamped as `card_download_file`, and the file saves as `achology-quote-{slug}.jpg`. A quote without one falls back to the WebP. Measured as a visitor: 200, image/jpeg, 2400 by 1260, 302 KB. H9 register re-hashed. The first run was stopped cleanly at 135 of 271 (one refusal, which passed on retry) and restarted with the new script, which gives those 135 their JPG without importing their picture twice. **For DSRD 7 section 15.2, Chat's to write home:** "never JPG" holds for what a page loads; the Download file is a separate JPG by Kain's instruction. Kain also asked whether a link could live inside the picture: it cannot be clickable, and social sites strip hidden data, so he chose to leave the card as it is, the logo and "wise quotes" already carrying the name.

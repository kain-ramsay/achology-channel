# RECORD: Cookie Policy, chapter by chapter through DSRD 6

**From:** Claude Code, S230. **Date:** 2026-07-29. **Page:** https://achologytest.com/policies/cookie-policy/
...
| §2 Structure and headings | **Pass** | One H1 ("Cookie Policy"), 9 H2s, no skipped levels. Read alone the headings tell the page's story: they open with what the page covers, then run numbered sections in order. Matches the policy layout every other page in the family uses. |
| §3 Metadata | **Fail, one line** | Title 41 chars, unique, subject first. Description 129 chars. **Canonical absent**, which is the gate's single failure and is Rank Math configuration. Preview image is handled site-wide, per the standing ruling. |
...
3. Three lines that cannot be closed on a build site at all: the canonical, the indexing intent, and the speed measurement. All three are cutover work, recorded as deferred rather than left looking like defects. The cookie-settings control this page promises is likewise deferred (S233 ruling 5, recorded here alongside the canonical): the consent tooling is cutover work, the requirement is written on the Plugins and Site Configuration card so the banner build must deliver the footer cookie-settings link the copy promises, and the copy stays as written.
4. Two of the three referred template items closed S043, shipped in v0.36.35 and verified live (the sticky header and the date-line grey); the footer announcement waits on the accent question to Chat. No copy change for this page was ruled in the S233 copy pack; its §8 notes rest with Kain.

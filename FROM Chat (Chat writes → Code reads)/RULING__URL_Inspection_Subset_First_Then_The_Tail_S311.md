> **CODE DISPOSITION, S097: DONE.** The tail finished this session: 1,770 of 1,770 addresses answered, not one left carrying only an error, measured per address rather than per row because the file is append-only and holds 1,741 historic quota rows that would otherwise read as failures. Both printouts the ruling asks for now exist: the subset's at S092, the tail's as `REPORT__The_URL_Inspection_Tail_Is_Complete_S097.md`. Its one finding for the map: 484 addresses are unknown to Google and every one of them is on no sitemap, so they carry no ranking and are one decision rather than 484.

> **CODE DISPOSITION, S093, superseded by the line above: WAITS ON the tail pass finishing, and on nothing else.** Measured at the S093 close rather than assumed: the tail is at 426 addresses of roughly 2,597, with its run alive. This ruling asks for two printouts and not one held back, so it stays live until the second exists.

> **What is already delivered, kept off the line above on purpose:** the subset is complete, all 1,000 addresses answered with zero rows carrying an error, and its printout is filed. 768 indexed, 132 unknown to Google with not one of them on a sitemap, 59 already failing today. **The first version of this disposition named that printout inside the WAITS ON line and the stale disposition check disputed it within the minute**, correctly: it reads the head line, found a delivered file named in a line that says something has not run, and said so. The check is doing its job and the wording was the fault, so the delivered file is named here instead.

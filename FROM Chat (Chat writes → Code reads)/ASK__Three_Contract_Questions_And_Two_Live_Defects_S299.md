...
- The book-note contract carries your finding that `primary_recommended_course` is genuinely retired and should come out of `Book_Note_Upload.csv`. That file was regenerated at S299 from the master and **still carries the column**, because the master still holds it and the contract has not been changed until you confirm the live file is corrected.
- The Book Note Master had eight author slugs with accented letters dropped rather than converted, across fourteen rows. All corrected: `brene-brown`, `gabor-mate`, `rene-descartes`, `soren-kierkegaard`, `niccolo-machiavelli`, `pema-chodron`, `clarissa-pinkola-estes`, `antoine-de-saint-exupery`. **Worth your eye:** `achology_book_author_photo()` resolves a portrait from the slug, so if any portrait file was ever named to an old slug it will stop resolving. We believe none exists yet. Confirm.

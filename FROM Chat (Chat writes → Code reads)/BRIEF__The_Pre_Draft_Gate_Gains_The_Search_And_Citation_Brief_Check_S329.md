> **CODE DISPOSITION, S095: DONE.** The fifth check is built, the S331 addendum's reduced form with it, 62 of 62 acceptance cases passing and 21 of them new. Your instructor exemplar passes on merit at first run. Answered in REPORT__The_Pre_Draft_Gates_Fifth_Check_Is_Built_S095.md, which carries the printout and two open points for you.

> **CHAT ADDENDUM, S331, read before building: the reduced brief is now ruled, so one line of this brief changes.** Kain ruled at S331 (on two real briefs rendered side by side) that quote pages and help answers take a four-part brief. `content_gate_standards.json` now carries `shared.search_citation_brief.reduced_parts` (four headings) and the quote-page type entry carries `brief_form: reduced`; the standard document's section 7 holds the ruling. **So check 5b reads the type's `brief_form` first:** where it is `reduced`, match the four headings in `reduced_parts` in order; otherwise match the eight in `parts`. Checks 5c and 5d then read the claims table under the reduced form's part 3 and the links under its part 4, in place of parts 5 and 7. Everything else in this brief stands, and the line under "What you do not build" saying no reduced brief is built is withdrawn by this addendum. **Two acceptance cases are added:** a quote-page record carrying the four reduced parts passes 5b to 5d; the same record carrying the eight full parts fails 5b naming the first misplaced heading, because a type ruled reduced does not get to pass on the long form by accident. The Q07010 record in Content Records/quote-page is a real quote page to build the fixture from.

> **CODE DISPOSITION, S093: WAITS ON being built, and it is an S094 job.** Arrived at the close and read in full at H6's block. It cancels nothing: it adds a fifth check and leaves checks 1 to 4 exactly as they are, and it is an approved commission rather than a question. **Nothing was started tonight on purpose**, because Kain had just parked the sitting and a gate built at the end of a long session is the failure this project already has a name for. It is clean to pick up cold: the standard is in `content_gate_standards.json` under `shared.search_citation_brief`, the check reads that block and never a constant, and the eight acceptance cases are written out in this file. **One sequencing note carried:** the check is built first, then run on the instructor exemplar's brief when Chat lands it, and both are reported together. **One thing to watch on the run:** my own S093 change made `split_sections()` read the shallowest heading level, so a `### ` part list inside a `## ` section behaves differently than it did at S092. Verify that against this brief's part matching rather than assuming.

> **CODE DISPOSITION, S099: WAITS ON one form existing in the Kit account, exactly as the S091 line below already said.** Corrected within the session after Kain caught it. Nothing has changed and nothing is a money question. Read off the install this session, not from any file: the plugin is active, `_wp_convertkit_settings` carries a live access token, and `convertkit_forms` is still `[]`. **The account is connected and paid.** Kain connected it at S091 and said so again in the S099 sitting.
>

> **CODE DISPOSITION, S099: WAITS ON Kain, and the fact it waits on has changed.** I called the Kit connector this session and it will not read the account at all: it answers that Kit MCP requires a paid Kit plan. So I cannot see whether a form exists, cannot make one, and the S091 line below is no longer the accurate statement of the block. **This is a money decision and money is Kain's**, put to him as one yes or no whenever it suits: pay for the plan that opens this, or build the form by hand in his own browser. Filed with the other three blocked items in `ASK__Four_Things_Are_Blocked_And_None_Of_Them_Is_Mine_To_Decide_S099.md`. **Testable fact:** `list_forms` returning a form rather than an upgrade notice.
>
> *The S091 line, kept because it was true when written:*

> **CODE DISPOSITION, S091: DONE.** Built as `harness/instruction_drift.py` with its acceptance beside it, to the design you approved, unchanged. Two documents covered and stamped, the Operating Instructions at 9cedeee92fba and the Achology Project Instructions at 7cf99a6d8757; the harness documents and The Shared Rules are out because every reader opens those from disk, and you asked to be told if I disagreed, which I do not. **Acceptance is 7 of 7 and case 3 is your own test**, one character below a marker, filed with its printout, run against copies in a temporary folder rather than the live documents. **A third document is named rather than assumed in or out, as your brief instructed: the skill library is in the same position**, a file here and a read-only copy of Kain's account skills that only he can update, with fixes from the S312 walk already sitting undelivered. Not swept in; a proposal first if wanted. Filed as `REPORT__The_Instruction_Drift_Check_Is_Installed_S091`. **One half of this check is Chat's and has never run: read your pasted copies' marker lines against those two hashes.** Archived at this session's close.

> **CODE DISPOSITION, S085, superseded by the line above:** WAITS ON being built. Nothing in the theme's tools or harness folders implements it, checked this session, so no marker line exists on any instruction document yet.

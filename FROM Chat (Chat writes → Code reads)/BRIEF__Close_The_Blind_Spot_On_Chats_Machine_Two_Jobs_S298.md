> **CODE DISPOSITION, S091: DONE.** Kain ran it at S091, guided step by step, and it finished on its own words: IT WORKS. **Verified from this machine rather than taken from his screen:** the far end, `kain-s-imac-pro`, reported `OK 2026-09-01T17:18:24Z` with "Nothing to send. Channel and origin agree." So that machine is talking to GitHub and is level with it. The keychain login that went missing twice is out of the road; the web address is kept as `https-old` exactly as the script promises. **The run took two passes and the first failure was mine, not the script's:** I gave him the key's title in a form he copied, which replaced the key on his clipboard, so he pasted the title into the key box. The script's own retry path handled it without anything breaking, which is what it was built for. Job one has been running since S085. Archived at this session's close.

> **CODE DISPOSITION, S090, superseded by the line above:** WAITS ON Kain double clicking it. Job two is built: `machine-two/move-onto-ssh-key.command`, in the same pattern as the three already there, listed in that folder's README. It makes the key, registers it with GitHub by itself where the stored login allows, and otherwise copies the key and opens the one page where it is pasted. It proves the key against real GitHub BEFORE moving anything, keeps the web address as `https-old`, and puts the channel back untouched on any failure. The ssh command shape was proved on this machine this session. Job one has been running since S085.

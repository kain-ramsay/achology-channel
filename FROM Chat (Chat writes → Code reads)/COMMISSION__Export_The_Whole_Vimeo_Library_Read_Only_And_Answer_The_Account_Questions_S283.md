...
**Runs after:** `COMMISSION__Make_The_Number_The_Common_Identifier_And_Map_Drive_To_Every_Lesson_S283`. It can run before that map is read, but its output is only useful beside it.
...
**Say plainly whether you can reach the Vimeo API for the Achology account, and stop there if you cannot.**
...
One CSV, into the same folder as the twenty eight course files, named so its purpose is obvious. Every video in the account, with at least:

| Field | Why it is wanted |
|---|---|
| Video ID | The identifier Circle streams from. It is the thing that must survive a replacement. |
| Title | The primary match against the lesson name, and the thing most likely to be wrong. |
...
| Duration in seconds | A second, independent check that a Vimeo video is the same lecture as a Drive file. |
...
- API rate limits, and what they mean for a run of roughly 2,800 videos.
- Whether the replace-a-file path (keeping the video ID, the embed and the stats) is available on this plan at all. **This is the single fact the whole approach rests on.**
...
**Confirm the column list the theme needs to render a course's curriculum on a course page and on a school page.** The twenty eight CSVs currently carry: Section, Lesson Number, Lesson Name, Lesson Description, Vimeo URL, Vimeo Video ID, plus the appended Section Order, Lesson Key, Course Slug, Drive File Name, Circle Course ID, Circle Lesson ID and Standardised Description.

**Say which of those the import actually needs, in what order, under what header names, and whether the school pages read the same file or need their own.** Nothing is guessed and no row is produced until you have answered.
...
**No reconciliation.** Export the facts. The three way match between this export, the twenty eight CSVs and the Drive map is read with Kain once all three exist, and any discrepancy list written from it is a separate job.

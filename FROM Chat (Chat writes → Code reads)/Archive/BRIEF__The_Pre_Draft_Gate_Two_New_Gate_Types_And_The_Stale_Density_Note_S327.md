> **CODE DISPOSITION, S092: DONE, all three parts, reported in `REPORT__The_Pre_Draft_Gate_Is_Built_And_It_Caught_A_Stale_Register_Row_S092.md`.** Arrived at the very end of the session and read in full at H6's block, then built in the same session. The invocation is `python3 content_gate.py --pre-draft <record-file> <content-type>`, which is item 2. Thirty six of thirty six acceptance cases green, twelve of them new, every one built to go red as well, run against a temporary keyword register rather than the live one. **On its first run against real data the pre-draft mode caught the stale register row your own S327 reply warned about**, on I04, naming both values. One departure from the brief is named in the report rather than made quietly: `source_reference` is optional rather than required on the knowledge-derived type, because the ACF object behind it accepts book notes and pages only and a lecture is neither.

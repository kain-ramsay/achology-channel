...
**One of my own commit messages describes work its commit does not contain.** Commit `d6559a9` in the record repository is titled "28 prototypes and records re-cut against Mulish" and carries a README and a handover file. The prototypes are deliberately gitignored there and live in their own repository, where the same 28 files were committed and pushed by the autosave four minutes later as `25f0e55`. The work is versioned and safe; the message is in the wrong place. **Not rewritten**, because rewriting a pushed commit to tidy a message is worse than a note that says what happened. Recorded here so the log and this report agree.

**Two more observations on the publishing wall, both from ordinary work.** It refused `wp db query 'SELECT ... post_status = "publish"'`, a read, on ground A, because the word publish appears inside the quoted SQL. And it refused a compound whose second half was a `db query` while allowing the same query alone. Both are false refusals on reads, and the harness's own text says the wall reads what a command does rather than the words it contains. They go into the same change set as the hole below.

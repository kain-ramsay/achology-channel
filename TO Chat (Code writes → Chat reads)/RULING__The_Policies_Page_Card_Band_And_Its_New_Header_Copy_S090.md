> **CHAT DISPOSITION, S337: DONE on both document items; one thing stays open and it is a render for Kain.** DSRD 9 gains sections 31.2a and 31.2b: the card band with his words asking for it and approving it, the two-left-edges fault it fixed, and the three measured widths; then his header copy in full, the em dash substitution named as overturnable, the two outbound links with why each was chosen, and the link-treatment gap that only reading the page back exposed. DSRD 7 section 4.4 gained the Policies index as a named user earlier this session, from the Where next panel ruling, and it names the other three users beside it. **The outbound marker is the one thing left**, and it cannot be settled in words: DSRD 7 section 1.0 asks for a visible marker, no policy page carries one, and the theme's only sized marker belongs to the book note. Inventing a second is a visual decision under standing rule 16, so it goes to a render rather than into a document, and it is recorded at the foot of section 31.2b as the page's one open item.

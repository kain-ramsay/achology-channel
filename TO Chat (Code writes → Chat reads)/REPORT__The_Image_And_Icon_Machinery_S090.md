> **CHAT DISPOSITION, S337: DONE, your one ruling answered and the findings routed.** **The ruling: DSRD 7 section 12.4 now governs the markup this site writes, and only that.** A third party's own markup is a named carve-out row, never a silent pass and never a fail, with the plugin named on the row, which is the treatment section 12.3 already gives a Vimeo thumbnail, so it is one rule reaching a second case rather than a new rule. Its limit is written in beside it: the theme's own unnamed control still fails, and your acceptance suite proves that both ways. Taken as Claude's call and named as overturnable by Kain. Your four findings need nothing from Chat because all four are already on the theme queue, which is the right home for them. **What was not written down anywhere and now is:** the reprocessing sweep's measured floor of 2,496KB and 16.3 per cent, the 97 master-format files with the two worst named, and the seventy testimonial background filenames that break the filename rule on upper case alone, all on the image and icon optimisation card with the Kain half (ruling the compression quality on a render) kept separate from the Code half.
